...
function AnySignApi() {
	...
	*/
    ...
    //    this.addEvidenceV2 = function(context_id,content, bioType,index)
    //
    //    {
    //        return core._addEvidenceV2(context_id,content,bioType,index);
    //    }
    ...
    this.getVersion = function () {
        return "AnySign_V1.3.1_Web_2.0.0.25515";
    }
...
function SignatureConfig(signer, signRule) {
    ...
    if (!(signRule instanceof SignRule_KeyWord) && !(signRule instanceof SignRule_Tid) && !(signRule instanceof SignRule_XYZ) && !(signRule instanceof SignRule_KeyWordV2) && !(signRule instanceof SignRule_KeyWordV3)) {
    ...
    this.singleWidth = 200; //(只针对签名)生成的签字最大宽度(不排除实际签名宽度小于此值)，单位像素
    this.singleHeight = 160; //(只针对签名)生成的签字最大高度(不排除实际签名高度小于此值)，单位像素
 ...
 * @constructor
 */
var SignRule_KeyWord = function (keyword, keyWordAlignMethod, keyWordOffset, pageNo) {
    if (!keyword || keyword.length === 0) {
        throw "SignRule_KeyWord constructor parameter keyword could not be null or empty";
    }

    if (keyWordAlignMethod !== "0" && keyWordAlignMethod !== "1" && keyWordAlignMethod !== "2" && keyWordAlignMethod !== "3") {
        throw "SignRule_KeyWord constructor parameter keyWordAlignMethod invalid, should be one of '0' '1' '2' '3'";
    }

    this.RuleType = "0";

    if (!isNaN(pageNo) && !isNaN(keyWordOffset)) {
        this.KWRule = {
            KW: keyword,
 ...
 * @constructor
 */
var SignRule_KeyWordV2 = function (keyword, keyWordAlignMethod, keyWordOffset, pageNo, KWIndex) {
    if (!keyword || keyword.length === 0) {
        throw "SignRule_KeyWordV2 constructor parameter keyword could not be null or empty";
    }

    if (keyWordAlignMethod !== "0" && keyWordAlignMethod !== "1" && keyWordAlignMethod !== "2" && keyWordAlignMethod !== "3") {
        throw "SignRule_KeyWordV2 constructor parameter keyWordAlignMethod invalid, should be one of '0' '1' '2' '3'";
    }
 ...
 * @constructor
 */
var SignRule_KeyWordV3 = function (keyword, xOffset, yOffset, pageNo, KWIndex) {
    if (!keyword || keyword.length === 0) {
        throw "SignRule_KeyWordV3 constructor parameter keyword could not be null or empty";
    }
    this.RuleType = "0";
